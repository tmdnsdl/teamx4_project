...
<script type="text/javascript">
	...
	function deleteUser(usernum,pageNumber){
		
		var result = confirm("정말로 삭제 하시겠습니까?");
		console.log(usernum);
		console.log(pageNumber);
		if(result){
			location.href="deleteuser.admin?usernum="+usernum+"&pageNumber="+pageNumber;
		}
		else{
			alert("취소되었습니다");
		}
...
<body >
   ...
   <div class="info_admin">
   		 <form action="user.admin" align="center">
    	<select name="column">
		...
		<table class="admin_table_2" width="100%">
		 <colgroup>
			...
			<tr class="admin_table_items">
				...
				<c:if test="${!fn:startsWith(user.user_image, 'http://k.kakaocdn.net/')}">src="<%=request.getContextPath()%>/resources/images/${user.user_image}"</c:if> style="width:100px; height:100px;">
				</td>   <!-- 파일 저장된곳 위치경로 지정해야함 -->
				<td >${user.user_name }</td>
				<td >${user.user_email }</td>
				<td >${user.user_gender }</td>
				<td><a href="#" onClick="deleteUser('${user.user_num}','${pg.pageNumber}')">
